...
if [[	${OPTIONS} = ?([BFM])?([BFM])?([BFM])?([BFM])?(' ')?(W?(' ')@(t|i))" S"?(' ')"qta1cu"[0-9][0-9]?(' 'C?(' ')+([._-a-zA-Z0-9]))     ||\
	${OPTIONS} = ?([BFM])?([BFM])?([BFM])?([BFM])?(' ')?(W?(' ')@(t|i))" "?(C?(' ')+([._-a-zA-Z0-9]))" S"?(' ')"qta1cu"[0-9][0-9]     ||\
	${OPTIONS} = ?([BFM])?([BFM])?([BFM])?([BFM])?(' ')S?(' ')"qta1cu"[0-9][0-9]?(' ')?(W?(' ')@(t|i))?(' 'C?(' ')+([._-a-zA-Z0-9]))  ||\
	${OPTIONS} = ?([BFM])?([BFM])?([BFM])?([BFM])?(' ')S?(' ')"qta1cu"[0-9][0-9]?(' 'C?(' ')+([._-a-zA-Z0-9]))?(' ')?(W?(' ')@(t|i))  ||\
	${OPTIONS} = ?([BFM])?([BFM])?([BFM])?([BFM])?(' ')?(C?(' ')+([._-a-zA-Z0-9]))?(' 'W?(' ')?([ti]))" S"?(' ')"qta1cu"[0-9][0-9]    ||\
	${OPTIONS} = ?([BFM])?([BFM])?([BFM])?([BFM])?(' ')?(C?(' ')+([._-a-zA-Z0-9]))" S"?(' ')"qta1cu"[0-9][0-9]?(' 'W?(' ')?([ti]))    ||\
	${OPTIONS} = ?(W?(' ')@(t|i))?(' '?([BFM])?([BFM])?([BFM])?([BFM]))?(' ')S?(' ')"qta1cu"[0-9][0-9]?(' 'C?(' ')+([._-a-zA-Z0-9]))  ||\
	${OPTIONS} = ?(W?(' ')@(t|i))?(' '?([BFM])?([BFM])?([BFM])?([BFM]))?(' ')C?(' ')+([._-a-zA-Z0-9])?(' ')S?(' ')"qta1cu"[0-9][0-9]  ]]
then
	...
	case $opt in
		...
		W )	WAIT_TO_SPLIT=YES
			...
			then
				...
				print Arguement passed for the W option was incorrect.
				print ${USAGE}
				print exiting
				exit
			fi
...
print Wait type is set to: ${WAIT_TYPE}
print "#"
